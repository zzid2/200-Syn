...
error_font="\033[31m[Error]$\033[0m "           ## 错误输出 红色
success_font="\033[32m[Success]\033[0m "        ## 成功输出 绿色
info_font="\033[36m[Info]\033[0m "              ## 显示输出 黄色

function echo_green_bg(){                   ## 绿色背景
	echo -e "\033[42;37m$1\033[0m"
}

function echo_yellow_bg(){                  ## 黄色背景
	echo -e "\033[43;37m$1\033[0m"
}

function echo_red_bg(){                     ## 红色背景
	echo -e "\033[41;37m$1\033[0m"
}

function clean_outdated_files(){           ## 清理过时文件
	rm -f "create_acl_for_luci.err" "create_acl_for_luci.warn" "create_acl_for_luci.ok"
}

function check_if_acl_exist(){              ## 检查acl是否存在
	ls "$1"/root/usr/share/rpcd/acl.d/*.json >/dev/null 2>&1 && return 0 || return 1
}

function check_config_files(){              ## 检查配置文件
	[ "$(ls "$1"/root/etc/config/* 2>/dev/null | wc -l)" -ne "1" ] && return 0 || return 1
}

function get_config_name(){                 ## 获取配置名称
	ls "$1"/root/etc/config/* 2>/dev/null | awk -F '/' '{print $NF}'
}

function create_acl_file(){                  ## 创建acl文件
	mkdir -p "$1"
	echo -e "{
	\"$2\": {
...
function auto_create_acl(){                   ## 自动创建acl
